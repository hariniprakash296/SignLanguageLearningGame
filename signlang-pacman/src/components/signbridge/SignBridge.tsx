...
import { Region, TranslationResult } from '@/lib/types';
import { REGIONS } from '@/lib/constants';
import { translateSign, generateSignVisual } from '@/lib/geminiService';
...
export const SignBridge: React.FC = () => {
    ...
    return (
        <div className="min-h-[calc(100vh-200px)] bg-[#eef2f6] flex flex-col font-sans selection:bg-indigo-100 selection:text-indigo-900 rounded-3xl overflow-hidden">
            <header className="bg-white/80 backdrop-blur-md border-b border-slate-200 sticky top-0 z-50 px-6 py-4">
                <div className="max-w-7xl mx-auto flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <div className="w-12 h-12 bg-gradient-to-tr from-indigo-700 to-blue-500 rounded-2xl flex items-center justify-center text-white shadow-xl shadow-indigo-200 rotate-3">
                            <Languages size={26} />
                        </div>
                        <div>
                            <h1 className="text-2xl font-black text-slate-900 tracking-tight leading-none">SignBridge</h1>
                            <p className="text-[10px] text-indigo-600 font-black uppercase tracking-[0.2em] mt-1">Advanced Syntax Decoder</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-4">
                        <div className="hidden sm:flex items-center gap-2 px-4 py-2 bg-white rounded-2xl border border-slate-200 text-xs font-bold text-slate-600 shadow-sm">
                        ...
                        <div className="space-y-8 animate-in fade-in slide-in-from-right-12 duration-1000">
                            ...
                            {visualUrl && (
                                <section className="bg-white p-8 rounded-[3rem] shadow-xl border border-white animate-in zoom-in-95 duration-700 delay-300">
                                    <h4 className="text-[10px] font-black text-slate-400 uppercase mb-5 tracking-widest">Visual Reference</h4>
                                    <div className="aspect-square rounded-[2rem] overflow-hidden bg-slate-100 border-4 border-slate-50 shadow-inner">
                                        <img src={visualUrl} alt="Sign visualization" className="w-full h-full object-cover" />
                                    </div>
            ...
            <footer className="py-16 px-6">
                <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-10">
                    <div className="flex items-center gap-4 text-slate-400">
                        ...
                        <p className="text-sm font-black tracking-tight">SignBridge Regional Pro • 2024</p>
                    </div>
                    <div className="flex gap-10">
                        <a href="#" className="text-xs font-black text-slate-400 hover:text-indigo-600 uppercase tracking-[0.2em] transition-colors">Linguistics</a>
                        <a href="#" className="text-xs font-black text-slate-400 hover:text-indigo-600 uppercase tracking-[0.2em] transition-colors">Privacy</a>
                        <a href="#" className="text-xs font-black text-slate-400 hover:text-indigo-600 uppercase tracking-[0.2em] transition-colors">Support</a>
                    </div>
                </div>
            </footer>
        </div>
